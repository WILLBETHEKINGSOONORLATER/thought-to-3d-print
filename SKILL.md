...
description: 想啥印啥 —— 把「文生图 → 图生3D → 在 Bambu Studio（拓竹切片软件）里打开」一次走通。适用于想把 AI 生成的 3D 模型直接送到拓竹 App 里查看、调参、打印的场景。规则：每产生一个可看的新产出（参考图 / 3D 模型 / 待打开的文件）都要先告知用户、等用户点头再继续，不许一路自动到底。边界明确：交付到「模型在拓竹 App 里成功打开」为止，不做切片。零依赖，不需要用户安装任何 Python 库。Turn a text prompt into a 3D model opened in Bambu Studio, notifying the user at each new deliverable, with no extra dependencies.
agent_created: true
---
...
**能做**：文生图 → 去水印 → 图生3D → 摆正 → 导出 → 在拓竹 App 里打开（每个关键产出都会先给用户看）。
...
| **2** | 图生3D 完成 | 模型文件 / 预览图 / 尺寸 | 「3D 模型出来了，形状对吗？可以我就去摆正、准备打开。」 |
| **3** | 摆正完成、准备打开 | 尺寸 + 对照机型表是否装得下 | 「模型是 115×92×117 mm，你的 H2C 装得下。现在在拓竹 App 里打开吗？」 |
